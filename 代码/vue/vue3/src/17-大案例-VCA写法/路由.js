...
import Films from '../17-大案例-VCA写法/views/Films.vue'
// import Cinemas from '../16-大案例/views/Cinemas.vue'
// import Center from '../16-大案例/views/Center.vue'
import NotFound from '../17-大案例-VCA写法/views/NotFound.vue'
import nowplaying from '../17-大案例-VCA写法/views/films/nowplaying.vue'
import comingsoon from '../17-大案例-VCA写法/views/films/comingsoon.vue'
import detail from '../17-大案例-VCA写法/views/Detail.vue'
import Login from '../17-大案例-VCA写法/views/Login.vue'
...
const routes = [
    ...
    {
        path: "/cinemas",
        component: () => import('../17-大案例-VCA写法/views/Cinemas.vue')      //上面静态导入，注释掉改成动态导入，实现懒加载，避免用户打开首页时，全部加载导致首页显示过慢
    },
    {
        path: "/center",
        component: () => import('../17-大案例-VCA写法/views/Center.vue'),
        meta: {
    ...
    {
        path: '/detail/:myid',         //需要写成动态的，因为点击跳转是后面带着id跳转的，：_占位符，myid是随便起的名
        name: 'Detail',
        component: detail
    },
...
//     // next({ name: 'Login' })：如果上述两个条件都满足，则重定向到名为 'Login' 的路由。         else next()：否则，继续导航到原本的目标路由。
...
router.beforeEach(async (to, from, next) => {         
    ...
    //后置钩子，做一些用户行为的分析，数据分析，和一些辅助作用，比如电影网站，收集客户点了哪些电影比较多，来判断哪些电影比较火
