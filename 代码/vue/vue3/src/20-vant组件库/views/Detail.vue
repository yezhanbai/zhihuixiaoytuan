...
const store = useTabbarStore()      //重要：这里把useTabbarStore里面的state，actions等方法都引进来了，所以写法三可以直接点到change()方法，写法一同理
...
onBeforeMount(()=>{
    // store.isTabbarShow = false    //写法1
    //pinia提供的写法：打补丁，传给state，同名会进行覆盖，值也会新的覆盖旧的，此方法适合需要同时修改多个的场景      //写法2
    store.$patch({
        isTabbarShow:false      
    })
}) 
onBeforeUnmount(()=>{
    ...
    // store.$reset()           //重置为默认值
    store.change(true)          //写法3，要先取定义actions
})
...
onBeforeRouteUpdate((to, from) => {         //在当前路由改变，但是该组件被复用时调用，可以访问this
